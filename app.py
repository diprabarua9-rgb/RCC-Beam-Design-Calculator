...
if term <= 0:
    ...
    st.success(f"Required Steel Area (As) = {As:.2f} mm²")
    # ======================================
...
if Vu_N <= phiVc:
    ...
    st.success("Concrete alone is adequate. Minimum stirrups required.")
    ...
    st.warning("Shear reinforcement is required.")

    st.write(f"Steel Shear (Vs) = {Vs/1000:.2f} kN")
    # ======================================
# STIRRUP DESIGN
# ======================================
...
st.success(f"Provide {int(legs)}-Legged Φ{int(stirrup_dia)} Stirrup")

st.success(f"Spacing = {spacing:.0f} mm c/c")
# ======================================
# DEVELOPMENT LENGTH
# ======================================
...
# Simplified BNBC Development Length
Ld = (fy * bar_dia) / (4 * 1.25 * math.sqrt(fc))
...
number_bar = math.ceil(As/area_one_bar)

As_provided = number_bar*area_one_bar

st.write(f"Area of One Bar = {area_one_bar} mm²")

st.write(f"Required Number of Bars = {number_bar}")

st.success(f"Provide {number_bar} Φ{selected_bar} mm Bars")

st.success(f"Provided Steel Area = {As_provided:.2f} mm²")
# ======================================
...
# Stirrup
stirrup = plt.Rectangle((cover,cover),
                        b-2*cover,
                        h-2*cover,
                        fill=False,
                        linewidth=2,
...
# Bottom Bars
x_start = cover + bar_dia

x_end = b - cover - bar_dia
...
# Top Bars (2 Nos)
top1 = plt.Circle(
    (cover+bar_dia, h-cover-bar_dia),
    bar_dia/2,
    color='blue'
)

top2 = plt.Circle(
    (b-cover-bar_dia, h-cover-bar_dia),
    bar_dia/2,
    color='blue'
)
